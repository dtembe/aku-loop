...
### Steering Dan: Upstream and Downstream
...
- Dan achieves eventual consistency through iteration
...
- Dan is going off track (implementing wrong things, duplicating work)
...
Regeneration cost: one Planning loop. Cheap compared to Dan going in circles.
...
- **Dan-Friendly Work Branches**: Scoped planning per branch for parallel work streams
- **JTBD → Story Map → SLC Release**: Product-focused planning with Simple/Lovable/Complete releases

These are documented for reference but not required for core Dan functionality.
